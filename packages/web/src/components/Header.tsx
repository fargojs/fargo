...
export default function Header() {
  ...
  return (
    <header className="sticky top-0">
      <Disclosure
        as="nav"
        className="max-w-7xl w-full mx-auto flex justify-between items-center h-[65px] border-b border-b-neutral-300">
        {({ open }) => (
          <>
            <NavLink to="/">
              <svg
            ...
            <Disclosure.Button className="sm:hidden">EYEYE</Disclosure.Button>
            <Disclosure.Panel className="sm:hidden">{JSON.stringify(zotera)}</Disclosure.Panel>
          </>
        )}
      </Disclosure>
    </header>
